        ...
        if (!m_fDrawn) {
            if (m_renderingCaches.outlineCache.Lookup(overlayKey, m_pOutlineData)) {
                ...
                if (m_style.borderStyle == 0 && (m_style.outlineWidthX + m_style.outlineWidthY > 0)) {
                    int rx = std::max<int>(0, lround(m_style.outlineWidthX));
                    int ry = std::max<int>(0, lround(m_style.outlineWidthY));
...
bool CWord::CreateOpaqueBox()
{
    ...
    int w = lround(m_style.outlineWidthX);
    int h = lround(m_style.outlineWidthY);
...
void CWord::Transform_C(const CPoint& org)
{
    ...
    for (ptrdiff_t i = 0; i < mPathPoints; i++) {
        ...
        // round to integer
        mpPathPoints[i].x = lround(x) + org.x;
        mpPathPoints[i].y = lround(y) + org.y;
    }
}
...
bool CPolygon::GetPOINT(LPCWSTR& str, POINT& point) const
{
    ...
    point.x = lround(wcstod(str, &xEnd) * m_scalex) * 64;
    if (xEnd <= str) {
        return false;
    }
    point.y = lround(wcstod(xEnd, &yEnd) * m_scaley) * 64;
...
bool CPolygon::ParseStr()
{
    ...
    int baseline = lround(m_scaley * m_baseline) * 64;
    m_descent = baseline;
    m_ascent -= baseline;
...
void CRenderedTextSubtitle::ParseEffect(CSubtitle* sub, CString str)
{
    ...
    if (!effect.CompareNoCase(_T("Banner;"))) {
        ...
        e->param[0] = lround(std::max(1.0 * delay / sub->m_scalex, 1.0));
        e->param[1] = lefttoright;
        e->param[2] = lround(sub->m_scalex * fadeawaywidth);
...
CSubtitle* CRenderedTextSubtitle::GetSubtitle(int entry)
{
    ...
    if (m_bOverrideStyle) {
        ...
        if (m_bOverridePlacement) {
            ...
            stss.marginRect.bottom = lround(m_dstScreenSize.cy - m_dstScreenSize.cy * m_overridePlacement.cy / 100.0);
            ...
            stss.marginRect.top    = m_dstScreenSize.cy - (stss.marginRect.bottom + lround(stss.fontSize * 3.0));
            stss.marginRect.left   = lround(m_dstScreenSize.cx * m_overridePlacement.cx / 100.0 - mw / 2.0);
