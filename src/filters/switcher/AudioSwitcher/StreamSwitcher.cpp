...
HRESULT GetPeer(CStreamSwitcherFilter* pFilter, T** ppT)
{
    *ppT = nullptr;
    ...
    if (CComQIPtr<T> pT = (IUnknown*)pConnected) {
        *ppT = pT.Detach();
        return S_OK;
    }
...
#define CallPeerSeekingAll(call)                            \
    ...
    while (pos) {                                           \
        ...
        if (FAILED(pPin->ConnectedTo(&pConnected)))         \
            continue;                                       \
        if (CComQIPtr<IMediaSeeking> pMS = (IUnknown*)pConnected) {    \
...
#define CallPeerAll(call)                                   \
    ...
    while (pos) {                                           \
        ...
        if (FAILED(pPin->ConnectedTo(&pConnected)))         \
            continue;                                       \
        if (CComQIPtr<IMediaPosition> pMP = (IUnknown*)pConnected) {   \
...
HRESULT CStreamSwitcherInputPin::InitializeOutputSample(IMediaSample* pInSample, IMediaSample** ppOutSample)
{
    ...
    if (CComQIPtr<IMediaSample2> pOutSample2 = (IUnknown*)pOutSample) {
