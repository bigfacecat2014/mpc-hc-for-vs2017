...
STDMETHODIMP CmadVRAllocatorPresenter::CreateRenderer(IUnknown** ppRenderer)
{
    ...
    if (CComQIPtr<ISubRender> pSR = (IUnknown*)m_pMVR) {
...
{
    if (CComQIPtr<IBasicVideo> pBV = (IUnknown*)m_pMVR) {
        pBV->SetDefaultSourcePosition();
        pBV->SetDestinationPosition(v.left, v.top, v.right - v.left, v.bottom - v.top);
    }

    if (CComQIPtr<IVideoWindow> pVW = (IUnknown*)m_pMVR) {
...
STDMETHODIMP_(SIZE) CmadVRAllocatorPresenter::GetVideoSize(bool bCorrectAR) const
{
    ...
    if (!bCorrectAR) {
        ...
        if (CComQIPtr<IBasicVideo2> pBV2 = (IUnknown*)m_pMVR) {
...
{
    if (CComQIPtr<IMadVRCommand> pMVRC = (IUnknown*)m_pMVR) {
...
STDMETHODIMP CmadVRAllocatorPresenter::SetPixelShader2(LPCSTR pSrcData, LPCSTR pTarget, bool bScreenSpace)
{
    ...
    if (CComQIPtr<IMadVRExternalPixelShaders> pMVREPS = (IUnknown*)m_pMVR) {
        if (!pSrcData && !pTarget) {
...
{
    if (CComQIPtr<IMadVRInfo> pMVRI = (IUnknown*)m_pMVR) {
        int playbackState;
