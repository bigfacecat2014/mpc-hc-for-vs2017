...
STDMETHODIMP CDXRAllocatorPresenter::CreateRenderer(IUnknown** ppRenderer)
{
    ...
    CComQIPtr<ISubRender> pSR = (IUnknown*)m_pDXR;
    if (!pSR) {
        m_pDXR = nullptr;
        return E_FAIL;
    }
...
{
    if (CComQIPtr<IBasicVideo> pBV = (IUnknown*)m_pDXR) {
        pBV->SetDefaultSourcePosition();
        pBV->SetDestinationPosition(v.left, v.top, v.right - v.left, v.bottom - v.top);
    }

    if (CComQIPtr<IVideoWindow> pVW = (IUnknown*)m_pDXR) {
...
STDMETHODIMP_(SIZE) CDXRAllocatorPresenter::GetVideoSize(bool bCorrectAR) const
{
    ...
    if (!bCorrectAR) {
        ...
        if (CComQIPtr<IBasicVideo2> pBV2 = (IUnknown*)m_pDXR) {
