...
STDMETHODIMP CVMR9AllocatorPresenter::CreateRenderer(IUnknown** ppRenderer)
{
    ...
    *ppRenderer = nullptr;
    ...
    if (CComQIPtr<IAMVideoAccelerator> pAMVA = (IUnknown*)pPin) {
        HookAMVideoAccelerator((IAMVideoAcceleratorC*)(IAMVideoAccelerator*)pAMVA);
    }

    CComQIPtr<IVMRFilterConfig9> pConfig = (IUnknown*)pBF;
    if (!pConfig) {
        return E_FAIL;
    }
    ...
    CComQIPtr<IVMRSurfaceAllocatorNotify9> pSAN = (IUnknown*)pBF;
    if (!pSAN) {
        return E_FAIL;
    }
