...
STDMETHODIMP CEVRAllocatorPresenter::CreateRenderer(IUnknown** ppRenderer)
{
    ...
    *ppRenderer = nullptr;
    ...
    CComQIPtr<IMFGetService, &__uuidof(IMFGetService)> pMFGS = (IUnknown*)pBF;
    CComQIPtr<IEVRFilterConfig> pConfig = (IUnknown*)pBF;
    if (SUCCEEDED(hr)) {
    ...
    CComQIPtr<IMemInputPin> pMemInputPin = (IUnknown*)pPin;
...
bool CEVRAllocatorPresenter::GetImageFromMixer()
{
    ...
    while (SUCCEEDED(hr)) {
        ...
        if (FAILED(GetFreeSample(&pSample))) {
            TRACE_EVR("EVR: no free sample now.\n");
            break;
        }

        TRACE_EVR("EVR: sample count = %d\n", ++counter);
        ...
        switch (dwObject) {
            ...
            case WAIT_TIMEOUT: {
                if (GetRenderersData()->m_iDisplayStats) {
                    ...
                    //        if (CComQIPtr<IMPCVideoDecFilter2> MPCVideoDecFilter = (IUnknown*)pFilter) {
                ...
                if (m_rtTimePerFrame == 0 && bDoneSomething) {
                    ...
                    if (
                        ...
                        if (SUCCEEDED(pPin->ConnectedTo(&pPinTo)) && pPinTo) {
                            m_Decoder = GetFilterName(GetFilterFromPin(pPinTo)); //获取解码器名称，用于HUD显示
                        }
                    }
...
HRESULT CEVRAllocatorPresenter::OnSampleFree(IMFAsyncResult* pResult)
{
    ...
    if (SUCCEEDED(hr)) {
        if (CComQIPtr<IMFSample> pSample = (IUnknown*)pObject) {
