...
void CDX9AllocatorPresenter::DrawStats()
{
    ...
    if (m_pLine && iDetailedStats) {
        ...
        // Draw background
        DrawRect(RGB(0, 0, 0), 64, CRect(int(topLeftX),
                                         int(topLeftY),
        ...
        if (m_rtTimePerFrame) {
            ...
            for (int i = 1; i <= NB_JITTER; ++i) {
                ...
                //float jitter = float(m_pllJitter[nIndex] - m_fJitterMean); // origin version, 平均数，对于25FPS，则通常为40 0000百纳秒
                float jitter = float(m_pllJitter[nIndex] - 400000); // modified by yxs, keep the same algorithm with SyncOffset.
                points[i - 1].x = topLeftX + i * gridStepX;
                points[i - 1].y = topLeftY + (jitter * textScale / 2000.0f + graphHeight / 2.0f); // 1ms == 5个像素，总高度=360 pixel,上半y轴等于180像素，最大36ms正偏差。
            }
            ...
            if (m_bSyncStatsAvailable) {
                ...
                for (int i = 1; i <= NB_JITTER; ++i) {
                    ...
                    float jitter = float(m_pllSyncOffset[nIndex] + 50000); // -(-5ms)，StdDev大约为-5ms，抖动不大，但是等待绝对时间在-5ms左右振荡。
                    float offset = float(m_pllSyncOffset[nIndex]); // 可用于显示每帧（从调用PresentEx的前一刻到第一行扫描线出现）的绝对等待时间。
                    points[i - 1].x = topLeftX + i * gridStepX;
                    // points[i - 1].y = topLeftY + (jitter * textScale / 2000.0f + graphHeight / 2.0f); // 观察抖动
                    points[i - 1].y = topLeftY + (offset * textScale / 2000.0f + graphHeight / 2.0f); // 观察每帧的绝对等待时间（垂直同步导致的额外开销时间）。
                }
