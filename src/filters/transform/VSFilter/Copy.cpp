...
void CDirectVobSubFilter::PrintMessages(BYTE* pOut)
{
    ...
    CString inputSubType = Subtype2String(m_pInput->CurrentMediaType().subtype);
    CString outputSubType = Subtype2String(m_pOutput->CurrentMediaType().subtype);
    msg.Format(_T("in: %dx%d %s\nout: %ldx%ld %s\n"), m_w, m_h, (LPCTSTR)inputSubType,
        bihOut.biWidth, bihOut.biHeight, (LPCTSTR)outputSubType);
