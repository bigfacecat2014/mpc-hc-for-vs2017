...
HRESULT CFGFilterVideoRenderer::Create(IBaseFilter** ppBF, CInterfaceList<IUnknown, &IID_IUnknown>& pUnks)
{
    ...
    if (m_clsid == CLSID_EVRAllocatorPresenter) {
        ...
        if (m_clsid == CLSID_EnhancedVideoRenderer) {
            ...
            if (CComQIPtr<IMFGetService> pMFGS = (IUnknown*)pBF) {
        ...
        BeginEnumPins(pBF, pEP, pPin) {
            if (CComQIPtr<IMixerPinConfig, &IID_IMixerPinConfig> pMPC = (IUnknown*)pPin) {
                pUnks.AddTail(pMPC);
                break;
            }
        }
    ...
    if (pCAP) {
        ...
        *ppBF = CComQIPtr<IBaseFilter>(pRenderer).Detach();
        pUnks.AddTail(pCAP);
        if (CComQIPtr<ISubPicAllocatorPresenter2> pCAP2 = (IUnknown*)pCAP) {
            pUnks.AddTail(pCAP2);
        }
    }
