...
static int ShowPPage(CAtlArray<Codec>& codecs, const CComboBox& box, HWND hWnd = nullptr)
{
    ...
    if (CComQIPtr<ISpecifyPropertyPages> pSPP = (IUnknown*)c.pBF) {
        CAUUID caGUID;
    ...
    } else if (CComQIPtr<IAMVfwCompressDialogs> pAMVfWCD = (IUnknown*)c.pBF) {
...
void CPlayerCaptureDialog::UpdateAudioControls()
{
    ...
    if (!m_pAMAIM.IsEmpty()) {
        ...
        for (size_t i = 0; i < m_pAMAIM.GetCount(); i++) {
            CComQIPtr<IPin> pPin = (IUnknown*)m_pAMAIM[i];
...
void CPlayerCaptureDialog::OnRecord()
{
    ...
    if (!m_pMainFrame->m_fCapturing) {
        ...
        CComQIPtr<IFileSinkFilter2> pFSF = (IUnknown*)m_pMux;
        if (pFSF) {
            m_pDst = m_pMux;
        } else {
            m_pDst = nullptr;
        ...
        if (m_fSepAudio && m_fAudOutput && m_pAudMux && !audfn.IsEmpty()) {
            ...
            CComQIPtr<IFileSinkFilter2> pFSFAudioMux = (IUnknown*)m_pAudMux;
            if (pFSFAudioMux) {
                m_pAudDst = m_pAudMux;
            } else {
                m_pAudDst = nullptr;
        ...
        if (CComQIPtr<IBufferFilter> pVB = (IUnknown*)m_pVidBuffer) {
        ...
        if (CComQIPtr<IBufferFilter> pAB = (IUnknown*)m_pAudBuffer) {
