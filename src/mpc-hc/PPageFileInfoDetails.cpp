    ...
    , m_creationDate(StrRes(IDS_AG_NOT_KNOWN))
{
    ...
    if (CComQIPtr<IBaseFilter> pBF = pFSF) {
        BeginEnumPins(pBF, pEP, pPin) {
            if (CComQIPtr<IAsyncReader> pAR = (IUnknown*)pPin) {
    ...
    if (pCAP) {
        ...
        if (wh.cx == 0 && wh.cy == 0) {
            BeginEnumFilters(pFG, pEF, pBF) {
                if (CComQIPtr<IBasicVideo> pBV = (IUnknown*)pBF) {
                    pBV->GetVideoSize(&wh.cx, &wh.cy);
                    if (CComQIPtr<IBasicVideo2> pBV2 = (IUnknown*)pBF) {
                        pBV2->GetPreferredAspectRatio(&arxy.cx, &arxy.cy);
                    }
                    break;
                } else if (CComQIPtr<IVMRWindowlessControl> pWC = (IUnknown*)pBF) {
                    pWC->GetNativeVideoSize(&wh.cx, &wh.cy, &arxy.cx, &arxy.cy);
                    break;
                } else if (CComQIPtr<IVMRWindowlessControl9> pWC9 = (IUnknown*)pBF) {
