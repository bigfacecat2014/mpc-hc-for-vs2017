...
bool CRealMediaPlayer::Init()
{
    ...
    CComQIPtr<IRMAAudioPushdown, &IID_IRMAAudioPushdown> pAP = (IUnknown*)m_pAudioPlayer;
    if (pAP) {
        pAP->SetAudioPushdown(300);    // 100ms makes the playback sound choppy, 200ms looks ok, but for safety we set this to 300ms... :P
    }

    CComQIPtr<IRMAErrorSinkControl, &IID_IRMAErrorSinkControl> pErrorSinkControl = (IUnknown*)m_pPlayer;
...
STDMETHODIMP CRealMediaPlayer::ErrorOccurred(const UINT8 unSeverity, const UINT32 ulRMACode, const UINT32 ulUserCode, const char* pUserString, const char* pMoreInfoURL)
{
    if (unSeverity < 5) {
        ...
        if (CComQIPtr<IRMAErrorMessages, &IID_IRMAErrorMessages> pErrorMessages = (IUnknown*)m_pPlayer) {
...
STDMETHODIMP CRealMediaPlayer::SitesNeeded(UINT32 uRequestID, IRMAValues* pProps)
{
    ...
    CComQIPtr<IRMAValues, &IID_IRMAValues> pSiteProps = (IUnknown*)m_pTheSite;
    if (!pSiteProps) {
        return E_NOINTERFACE;
    }
...
{
    if (CComQIPtr<IRMAVideoSurface, &IID_IRMAVideoSurface> pRMAVS = (IUnknown*)m_pRMAP) {
