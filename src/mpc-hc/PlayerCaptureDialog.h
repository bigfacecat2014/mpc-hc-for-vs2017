...
public:
    ...
    CFormat<T>* Find(CString name, bool fCreate = false) {
        for (size_t i = 0; i < CAutoPtrArray<CFormat<T>>::GetCount(); ++i) {
            if (CAutoPtrArray<CFormat<T>>::GetAt(i)->name == name) {
                return CAutoPtrArray<CFormat<T>>::GetAt(i);
            }
        }
        ...
        if (fCreate) {
            ...
            CFormat<T>* tmp = pf;
            CAutoPtrArray<CFormat<T>>::Add(pf);
            return tmp;
        }
    ...
    bool FindFormat(AM_MEDIA_TYPE* pmt, CFormat<T>** ppf) {
        ...
        for (size_t i = 0; i < CAutoPtrArray<CFormat<T>>::GetCount(); ++i) {
            CFormat<T>* pf = CAutoPtrArray<CFormat<T>>::GetAt(i);
