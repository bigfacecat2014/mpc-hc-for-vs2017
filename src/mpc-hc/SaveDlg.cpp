...
BOOL CSaveDlg::OnInitDialog()
{
    ...
    if (!pReader) {
        ...
        if (CComQIPtr<IBaseFilter> pSrc = (IUnknown*)pUnk) { // url reader has to be in the graph to load the file
    ...
    CComQIPtr<IBaseFilter> pSrc = (IUnknown*)pReader;
    ...
    CComQIPtr<IFileSinkFilter2> pFSF = (IUnknown*)pDst;
