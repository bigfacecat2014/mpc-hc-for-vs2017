                    ...
                    case PM_ANALOG_CAPTURE:
                        ...
                        if (m_fCapturing) {
                            if (m_wndCaptureBar.m_capdlg.m_pMux) {
                                CComQIPtr<IMediaSeeking> pMuxMS = (IUnknown*)m_wndCaptureBar.m_capdlg.m_pMux;
        ...
        case TIMER_STATS: {
...

            {
                ...
                BeginEnumFilters(m_pGB, pEF, pBF) {
                    ...
                    BeginEnumPins(pBF, pEP, pPin) {
                        if (CComQIPtr<IBitRateInfo> pBRI = (IUnknown*)pPin) {
            ...
            if (GetPlaybackMode() == PM_DVD) { // we also use this timer to update the info panel for DVD playback
                ...
                if (m_pDVBState->bActive) {
                    CComQIPtr<IBDATuner> pTun = (IUnknown*)m_pGB;
    ...
    if (GetLoadState() == MLS::LOADING) {
        ...
        if (!m_playingmsg.IsEmpty()) {
            ...
            if (SUCCEEDED(m_pCGB->FindPin(m_wndCaptureBar.m_capdlg.m_pDst, PINDIR_INPUT, nullptr, nullptr, FALSE, 0, &pPin))) {
                LONGLONG size = 0;
                if (CComQIPtr<IStream> pStream = (IUnknown*)pPin) {
                ...
                if (m_wndCaptureBar.m_capdlg.m_pVidBuffer
                    ...
                    if (CComQIPtr<IBufferFilter> pVB = (IUnknown*)m_wndCaptureBar.m_capdlg.m_pVidBuffer) {
                        nFreeVidBuffers = pVB->GetFreeBuffers();
                    }
                    if (CComQIPtr<IBufferFilter> pAB = (IUnknown*)m_wndCaptureBar.m_capdlg.m_pAudBuffer) {
            ...
            BeginEnumFilters(m_pGB, pEF, pBF) {
                if (CComQIPtr<IAMNetworkStatus, &IID_IAMNetworkStatus> pAMNS = (IUnknown*)pBF) {
...
void CMainFrame::SaveThumbnails(LPCTSTR fn)
{
    ...
    if (m_pCAP) {
        ...
        CComQIPtr<IBasicVideo2> pBV2 = (IUnknown*)m_pBV;
        long lARx = 0, lARy = 0;
...
void CMainFrame::OnPlayStop()
{
    ...
    if (GetLoadState() == MLS::LOADED) {
        if (GetPlaybackMode() == PM_FILE) {
            ...
            BeginEnumFilters(m_pGB, pEF, pBF) {
                CComQIPtr<IAMNetworkStatus, &IID_IAMNetworkStatus> pAMNS = (IUnknown*)pBF;
                CComQIPtr<IFileSourceFilter> pFSF = (IUnknown*)pBF;
                if (pAMNS && pFSF) {
                    WCHAR* pFN = nullptr;
                    AM_MEDIA_TYPE mt;
...
void CMainFrame::OnPlayFilters(UINT nID)
{
    ...
    CComQIPtr<IBaseFilter> pBF = (IUnknown*)pUnk;
    ...
    if (CComQIPtr<ISpecifyPropertyPages> pSPP = (IUnknown*)pUnk) {
    ...
    if (ps.GetPageCount() > 0) {
        ...
        if (bIsInternalFilter) {
            if (CComQIPtr<ILAVFSettings> pLAVFSettings = (IUnknown*)pBF) {
            ...
            } else if (CComQIPtr<ILAVVideoSettings> pLAVVideoSettings = (IUnknown*)pBF) {
                CFGFilterLAVVideo::Settings settings;
                if (settings.GetSettings(pLAVVideoSettings)) { // Get current settings from LAVVideo
                    settings.SaveSettings(); // Save them to the registry/ini
                }
            } else if (CComQIPtr<ILAVAudioSettings> pLAVAudioSettings = (IUnknown*)pBF) {
...
void CMainFrame::OpenCreateGraphObject(OpenMediaData* pOMD)
{
    ...
    if (CComQIPtr<IObjectWithSite> pObjectWithSite = (IUnknown*)m_pGB) {
        ...
        if (FAILED(hr)) {
            if (bMainFile) {
                if (s.fReportFailedPins) {
                    CComQIPtr<IGraphBuilderDeadEnd> pGBDE = (IUnknown*)m_pGB;
...
void CMainFrame::SetupChapters()
{
    ...
    while (pos && !m_pCB->ChapGetCount()) {
        ...
        BeginEnumPins(pBF, pEP, pPin) {
            ...
            if (CComQIPtr<IPropertyBag> pPB = (IUnknown*)pPin) {
                for (int i = 1; ; i++) {
                    CStringW str;
                    CComVariant var;
...
void CMainFrame::OpenCustomizeGraph()
{
    ...
    if (r.m_AdvRendSets.bSynchronizeVideo && s.iDSVideoRendererType == VIDRNDT_DS_SYNC) {
        ...
        if (SUCCEEDED(hr) && SUCCEEDED(m_pGB->AddFilter(m_pRefClock, L"SyncClock Filter"))) {
            CComQIPtr<IReferenceClock> refClock = (IUnknown*)m_pRefClock;
            CComQIPtr<IMediaFilter> mediaFilter = (IUnknown*)m_pGB;
            ...
            if (refClock && mediaFilter) {
                ...
                CComQIPtr<ISyncClockAdviser> pAdviser = (IUnknown*)m_pCAP;
                if (pAdviser) {
    ...
    if (GetPlaybackMode() == PM_DVD) {
        BeginEnumFilters(m_pGB, pEF, pBF) {
            if (CComQIPtr<IDirectVobSub2> pDVS2 = (IUnknown*)pBF) {
    ...
    BeginEnumFilters(m_pGB, pEF, pBF) {
        if (GetCLSID(pBF) == CLSID_OggSplitter) {
            if (CComQIPtr<IAMStreamSelect> pSS = (IUnknown*)pBF) {
...
void CMainFrame::OpenSetupVideo()
{
    ...
    if (m_pMFVDC) { // EVR
    ...
    } else {
        {
            ...
            if (CComQIPtr<IBasicVideo> pBV = (IUnknown*)m_pGB) {
        ...
        if (m_fAudioOnly) {
            BeginEnumFilters(m_pGB, pEF, pBF) {
                ...
                if (CComQIPtr<IVideoWindow> pVW = (IUnknown*)pBF) {
                    long lVisible;
...
void CMainFrame::OpenSetupCaptureBar()
{
    if (GetPlaybackMode() == PM_ANALOG_CAPTURE) {
        if (m_pVidCap && m_pAMVSCCap) {
            CComQIPtr<IAMVfwCaptureDialogs> pVfwCD = (IUnknown*)m_pVidCap;
        ...
        if (m_pAudCap && m_pAMASC) {
            ...
            BeginEnumPins(m_pAudCap, pEP, pPin) {
                if (CComQIPtr<IAMAudioInputMixer> pAIM = (IUnknown*)pPin) {
                    pAMAIM.Add(pAIM);
                }
            }
            EndEnumPins;
...
void CMainFrame::OpenSetupInfoBar(bool bClear /*= true*/)
{
    ...
    if (GetPlaybackMode() == PM_FILE) {
        ...
        BeginEnumFilters(m_pGB, pEF, pBF) {
            if (CComQIPtr<IAMMediaContent, &IID_IAMMediaContent> pAMMC = (IUnknown*)pBF) {
...
void CMainFrame::OpenSetupStatusBar()
{
    ...
    if (!m_fCustomGraph) {
        ...
        BeginEnumFilters(m_pGB, pEF, pBF) {
            CComQIPtr<IBasicAudio> pBA = (IUnknown*)pBF;
            if (!pBA) {
                continue;
            }
...
int CMainFrame::SetupAudioStreams()
{
    ...
    if (!pSS && m_pFSF) { // Try to find the main splitter
        ...
        if (!pSS) { // If the source filter isn't a splitter
            CComQIPtr<IBaseFilter> pBF = (IUnknown*)m_pFSF;
...
int CMainFrame::SetupSubtitleStreams()
{
    ...
    if (!m_pSubStreams.IsEmpty()) {
        ...
        while (pos) {
            ...
            CComQIPtr<IAMStreamSelect> pSSF = (IUnknown*)subInput.pSourceFilter;
            ...
            if (pSSF && CFGFilterLAV::IsInternalInstance(subInput.pSourceFilter)) {
                ...
                if (CComQIPtr<ILAVFSettings> pLAVFSettings = (IUnknown*)subInput.pSourceFilter) {
        ...
        if (GetPlaybackMode() == PM_FILE && pFileData) {
            ...
            if (m_pCAP2 && m_pFSF) {
                ...
                if (GetCLSID(pBF) == GUID_LAVSplitter || GetCLSID(pBF) == GUID_LAVSplitterSource) {
                    if (CComQIPtr<IPropertyBag> pPB = (IUnknown*)pBF) {
                        CComVariant var;
...
void CMainFrame::SetupFiltersSubMenu()
{
    ...
    if (GetLoadState() == MLS::LOADED) {
        ...
        BeginEnumFilters(m_pGB, pEF, pBF) {
            ...
            CComQIPtr<ISpecifyPropertyPages> pSPP = (IUnknown*)pBF;
            ...
            CComQIPtr<IAMStreamSelect> pSS = (IUnknown*)pBF;
            DWORD nStreams = 0;
...
void CMainFrame::SetupSubtitlesSubMenu()
{
    ...
    if (GetPlaybackMode() == PM_DIGITAL_CAPTURE) {
        ...
        while (pos) {
            ...
            if (CComQIPtr<IAMStreamSelect> pSSF = (IUnknown*)subInput.pSourceFilter) {
                DWORD cStreams;
...
bool CMainFrame::SetSubtitle(int i, bool bIsOffset /*= false*/, bool bDisplayMessage /*= false*/)
{
    ...
    if (pSubInput) {
        CComHeapPtr<WCHAR> pName;
        if (CComQIPtr<IAMStreamSelect> pSSF = (IUnknown*)pSubInput->pSourceFilter) {
            DWORD dwFlags;
...
void CMainFrame::SetSubtitle(const SubtitleInput& subInput)
{
...

    {
        ...
        if (subInput.pSubStream) {
            ...
            CComQIPtr<ISubRenderOptions> pSRO = (IUnknown*)m_pCAP;
...
{
    BeginEnumPins(pBF, pEP, pPin) {
        if (CComQIPtr<IAMBufferNegotiation> pAMBN = (IUnknown*)pPin) {
...
bool CMainFrame::BuildGraphVideoAudio(int fVPreview, bool fVCapture, int fAPreview, bool fACapture)
{
    ...
    if (m_pAudCap) {
        ...
        if (pMux != (IBaseFilter*)pAudMux && fACapture) {
    ...
    if ((m_pVidCap || m_pAudCap) && fCapture && fFileOutput) {
        ...
        if (CComQIPtr<IConfigInterleaving> pCI = (IUnknown*)pMux) {
        ...
        if (pMux != (IBaseFilter*)pAudMux && pAudMux != (IBaseFilter*)pAudDst) {
    ...
    switch (target) {
        ...
        case UPDATE_LOGO:
            if (GetLoadState() == MLS::LOADED && m_fAudioOnly && s.bEnableCoverArt) {
                ...
                CComQIPtr<IFilterGraph> pFilterGraph = (IUnknown*)m_pGB;
